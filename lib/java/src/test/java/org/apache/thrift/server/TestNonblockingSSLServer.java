...
import org.apache.thrift.transport.TNonblockingSSLServerSocket;
  ...
  public void startServer(
    ...
    serverThread =
        new Thread(
            () -> {
              try {
                TNonblockingSSLServerSocket tServerSocket =
                    TSSLTransportFactory.getNonblockingServerSocket(PORT);
                final Args args =
                    new Args(tServerSocket).processor(processor).protocolFactory(protoFactory);
                if (factory != null) {
